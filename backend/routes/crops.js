  ...
  try {
    ...
    if (!crop) {
      return res.status(404).json({ 
        error: 'Crop not found or you do not have permission to view it.' 
      });
    }
  ...
  try {
    ...
    if (!name || !plantingDate || !expectedHarvestDate) {
      return res.status(400).json({ 
        error: 'Crop name, planting date, and expected harvest date are required.' 
      });
    }
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    if (!crop) {
      return res.status(404).json({ 
        error: 'Crop not found or you do not have permission to edit it.' 
      });
    }
    ...
    res.status(500).json({ 
      error: 'Server error while updating crop.' 
    });
  }
});
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    res.json({
      message: 'Crop and associated tasks deleted successfully'
    });
    ...
    res.status(500).json({ 
      error: 'Server error while deleting crop.' 
    });
  }
});
...
router.get('/stats/dashboard', auth, async (req, res) => {
  try {
    ...
    res.status(500).json({ 
      error: 'Server error while fetching crop statistics.' 
    });
  }
});
